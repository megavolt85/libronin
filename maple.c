...
void maple_init()
{
	...
	MAPLE(0x14) = 1;
	dmabuffer = (void*)(((((unsigned long)(void *)&dmabuffer0)+31)&~31)|0xa0000000);
...
static struct mapledev *check_pads()
{
	...
	if(pending) 
	{
		...
		for(i=0; i<4; i++) 
		{
			...
			else if(*r == MAPLE_RESPONSE_DEVINFO && r[3]>=28 && (fc & (MAPLE_FUNC_CONTROLLER | MAPLE_FUNC_KEYBOARD | MAPLE_FUNC_MOUSE))) 
			{
				dev[i].xfunc = ((fc & MAPLE_FUNC_CONTROLLER) ?   (fc &(MAPLE_FUNC_CONTROLLER|MAPLE_FUNC_LIGHTGUN)) : 
																((fc & MAPLE_FUNC_MOUSE)? MAPLE_FUNC_MOUSE : MAPLE_FUNC_KEYBOARD));
				dev[i].xcond = (((dev[i].xfunc&0xff)<<24)|((dev[i].xfunc&0xff00)<<8)) & ~(MAPLE_FUNC_LIGHTGUN<<24);
				dev[i].func = 0;
				dev[i].ttl = 10;
